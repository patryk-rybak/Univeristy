...
class Jungle:
    ...
    def heuristic(self, me):
        ...
        return 100 * summary_dist + 100 * summary_weight 
...
class Player(object):
    ...
    def find_best(self, moves):
        ...
        def alphabeta(state, depth, alpha, beta, maximizingPlayer):
            if depth == 0 or state.check_finish() == self.my_player:
                return 100000
        ...
        for m in moves:
            ...
            temp.do_move(m)
            score = alphabeta(temp, 1, -inf, +inf, 1 - self.my_player)
            if score > best_score:
                best_score = score
                best_move = m
        return best_move
    ...
    def loop(self):
        while True:
            ...
            if moves:
                ...
                if not moves: moves = self.game.moves(self.my_player)

                move = self.find_best(moves)
                self.game.do_move(move)
